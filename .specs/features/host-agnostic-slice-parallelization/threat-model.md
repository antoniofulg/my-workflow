...
| Terminal send -> effect | `agent_prompt_stalled` or missing receipt hides a completed turn, prompting a blind resend or commit-only adoption | No retry or replacement; same-handle effect is accepted only after one unique marker and complete state proof | One-send turn ledger, bounded 250 ms / 300000 ms machine-only reconciliation, connected screen/tui-idle frames, HEAD/status/commit/gate/comment correlation, and serial recovery |
