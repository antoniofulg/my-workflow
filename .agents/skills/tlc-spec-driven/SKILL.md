...
3. One atomic commit per task. Mark the task complete in the current local `tasks.md` (and update spec traceability when used) **before** that commit; planning files under `.specs/features/` stay out of the commit. Never batch tasks; never weaken, skip, or delete tests to make them pass.
