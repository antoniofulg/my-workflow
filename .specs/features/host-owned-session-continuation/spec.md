...
4. The project decision `AD-019` SHALL supersede `AD-008`, name Orca only as rationale evidence, and establish the host-neutral ownership boundary. (HSC-16)
...
| State-transition integrity | `AD-008` becomes superseded only when `AD-019` is appended and the decision index is regenerated. |
...
| --- | --- | --- | --- |
| HSC-01 | Remove the published integration | Tasks | Verified |
