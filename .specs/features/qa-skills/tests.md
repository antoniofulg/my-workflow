...
| IT-014 | Feature workflow state is versioned | Evaluate Git visibility for feature specs and durable `.specs` files | `.specs/features/qa-skills/spec.md` and its task state are eligible for tracking alongside `.specs/STATE.md` and `.specs/AD-INDEX.md` |
| IT-015 | Versioned task state is the commit precondition | Inspect workflow instructions and commit ordering against the tracked `.specs/features/qa-skills/tasks.md` | The task gate closes the current task state before the atomic commit, and the task/status update may be included in that same commit |
| IT-016 | Feature packets remain durable | Inspect tracked files under `.specs/features/` after implementation | Feature specs, task contracts, decisions, and validation state remain eligible for tracking; disposable raw evidence stays under `docs/qa/evidence/` |
