...
| IT-028 | The stall bound, its default, and what the halt report names | read `docs/guidelines/REVIEW-ROUNDS.md` `## Escalation` | names `stall_attempts`, the default `3`, `0` as unbounded, and a halt report carrying the repeated signature and the attempt count |
