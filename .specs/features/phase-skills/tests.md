...
| --- | --- | --- | --- |
| UT-001 | Five phase skills exist with scoped frontmatter (PSK-01 AC1) | read `.agents/skills/w{specify,design,tasks,implement,verify}/SKILL.md` | each has frontmatter `name` equal to its directory, no `disable-model-invocation` key, a `description` containing the preloading agent name and `/w<phase>`, and the frontmatter parses under the repo's strict YAML reader (`tools/shared/src/frontmatter.ts` via `bun -e`) |
