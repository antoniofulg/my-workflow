...
| --- | --- | --- | --- |
| UT-001 | Large and Complex specs require Impact (SID-01 AC6, EC1) | fixture spec with header `Size: Large` and no `## Impact`; same with `Size: Complex`; same with `Size: Medium`; Large with `## Impact` body `none` | first two exit 1 naming `Impact`; Medium exits 0; `none` body exits 0 |
| UT-002 | Specify carries the new steps (SID-01 AC1, AC3; SID-02 AC1) | read `.agents/skills/wspecify/SKILL.md` | contains an `Impact` step before User Stories, a `uiux.md` step after Acceptance Criteria and before the closure gate, a gap-hunt step citing `references/gap-hunt.md`; the reference exists; SKILL.md ≤ 200 lines |
...
| IT-002 | Sync rejects a config missing the designer table (SID-03 AC4, EC3) | temp root whose toml lacks `[models.claude.designer]`; then one lacking `templates/agents/claude/designer.md` | each exits non-zero naming the table or template; nothing written |
