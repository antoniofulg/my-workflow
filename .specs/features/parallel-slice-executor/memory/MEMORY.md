...
- Grouped C-D review remediation binds Git consumers to one exact common directory, persists producer HEADs from owned worktrees, serializes malformed checkpoints with cleanup receipts, emits canonical `declared_paths`, and integrates only complete verified lanes. The real Orca pilot remains fresh-QA-owned and cleanup follows terminal status/ack/release evidence.
- Final grouped C-D remediation binds every lane Git effect to persisted path/gitdir/branch/HEAD ownership, requires an explicit fresh external Technical Verifier receipt before frozen-root integration, unions producer checkpoint paths, and uses a non-vacuous two-lane lifecycle proof before pilot cleanup. No real Orca pilot was run.
- QA fix loop keeps the disposable pilot as a worktree of the registered owner common directory, persists bounded structured Orca partial effects for exact Run/Task retry, and separates lifecycle-authorized normal cleanup from diagnostic abort. No QA pass or author Orca run is claimed.
