...
def test_public_state_correlates_identity_and_never_retries_send() -> None:
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        calls = root / "calls"
        fake = root / "orca"
        ...
        fake.chmod(0o755)
        marker = "POST_EFFECT_MARKER"
        request = root / "request.json"
        state = root / "state.json"
        payload = {
            "schema_version": 1, "repository": "repo-1", "repository_root": str(root),
            "slice_id": "S4", "task_id": "T6", "operation_id": "op-6",
            "terminal_handle": "term-1", "packet_path": str(root / "packet.md"),
            "packet_body": marker, "route": "route-6", "commit_id": "a" * 40,
            "lease_id": "lease-6", "log_path": str(root / "probe.jsonl"),
        }
        request.write_text(json.dumps(payload), encoding="utf-8")
        env = {**os.environ, "CALLS": str(calls)}
        command = [sys.executable, str(ROOT / "orca_assisted_probe.py"), "--orca", str(fake),
        ...
        assert sum(line.startswith("terminal send --terminal term-1 --text read ") for line in call_lines) == 1
        assert call_lines[0].endswith(" and execute it as your packet --enter --json")
        stored = json.loads(state.read_text(encoding="utf-8"))
        assert stored["operation_id"] == "op-6"
        assert stored["route"] == "route-6"
        assert stored["commit_id"] == "a" * 40
        assert stored["lease_id"] == "lease-6"
        assert marker not in first.stdout + second.stdout + calls.read_text(encoding="utf-8")
        ...
        assert "reused or changed" in rejected.stderr
        assert calls.read_text(encoding="utf-8").count("terminal send") == 1
...
def main() -> None:
    ...
    test_route_requires_two_consecutive_screen_frames_after_reset()
    test_cleanup_refuses_when_owned_branch_ref_remains()
    test_cleanup_stops_only_owned_handle_and_preserves_foreign_worktree()
    test_effect_reconciliation_accepts_one_same_handle_commit()
    test_sync_moves_dependent_lane_to_exact_producer_commit_and_gate()
    test_public_state_correlates_identity_and_never_retries_send()
    print("orca assisted probe contract: 8/8 passed")
