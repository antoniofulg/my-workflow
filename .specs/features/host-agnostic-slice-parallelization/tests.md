...
| IT-005 | AST-01–AST-07 | Assisted coordinator contract | Automatic Orca unsupported with explicit assisted authorization | One unique-name create is reconciled without blind retry; explicit-base worktree proves one new, uniquely owned, unused startup shell with no default/agent activity; a bounded machine-only exact-handle probe sees two consecutive connected `source=screen` frames matching the frozen route; dependency waiting remains event-driven with no model turns; same exact handle is reused; exact integrated worktree/branch/terminal absence is proven; no compatibility PASS; TLC/review/QA stages are preserved. |
