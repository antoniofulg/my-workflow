...
before an update. Run `python3 /path/to/my-workflow/scripts/adopt.py /path/to/target-project` only
after the review. For a filled product paragraph, pass `--skip-agents`. Adoption also runs the
